                ...
                if(this.ready()){
                    var username = this.request.body.username;
                        ...
                        if(username != "" && QuestionsAnswered.findOne({"userName": username})){
                            var resp = {"profile": Meteor.users.find({"profile.userName": username}, {fields: {"profile": 1}}).fetch(), "entries": QuestionsAnswered.find({"userName": username}, {fields: {"createdAt": 1, "question": 1, "correct": 1, "difficult": 1}}).fetch()}
